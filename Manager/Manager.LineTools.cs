...
namespace AutoPublicTransit
{
    ...
    public partial class Manager
    {
        ...
        private bool CancelActiveScanForDeleteAllLines(TransportInfo.TransportType transportType)
        {
            ...
            if (transportType == TransportInfo.TransportType.Metro && _metroScanRunning)
            {
                _metroScanRunning = false;
                TransitLogging.Warn("Delete all Metro lines requested during an active APT Metro scan; cancelling pending Metro validation settlement before deletion.");
                if (State.LastMetroScanSummary != null && !State.LastMetroScanSummary.Completed)
                {
                    State.LastMetroScanSummary.FailureMessage = "Cancelled by Delete Metro.";
                    AutoPublicTransitUI.UpdateMetroScanSummary(State.LastMetroScanSummary);
                }

                return true;
            }

            if (transportType == TransportInfo.TransportType.Train && _trainScanRunning)
            {
                _trainScanRunning = false;
                TransitLogging.Warn("Delete all Train lines requested during an active APT passenger Train scan; cancelling pending validation settlement before deletion.");
                if (State.LastTrainScanSummary != null && !State.LastTrainScanSummary.Completed)
                {
                    State.LastTrainScanSummary.FailureMessage = "Cancelled by Delete Train.";
                    AutoPublicTransitUI.UpdateTrainScanSummary(State.LastTrainScanSummary);
                }
