...
namespace AutoPublicTransit
{
    public class TransitRouteBuilder
    {
        ...
        private const int MaxRegionalConnectorRoutes = 4;
        private const int MaxRegionalConnectorAnchors = 160;
        ...
        public int RegionalConnectorRouteCount { get; private set; }
        public int RegionalConnectorAnchorCount { get; private set; }
        public int RegionalConnectorClusterCount { get; private set; }
        public int RegionalConnectorTransitHubAnchorCount { get; private set; }
        public int RegionalConnectorRejectedRouteCount { get; private set; }
        ...
        public List<List<Vector3>> BuildRegionalConnectorRoutes(
            List<ExistingLineSnapshot> publishedLines,
            List<Vector3> transitHubs,
            AutoPublicTransitConfig cfg,
            BusStopLocator stopLocator)
        {
            ResetRegionalConnectorDiagnostics();
            _planningLinkDistanceCache.Clear();

            var routes = new List<List<Vector3>>();
            if (cfg == null || stopLocator == null)
                return routes;

            List<RegionalConnectorAnchor> anchors = BuildRegionalConnectorAnchors(publishedLines, transitHubs, cfg, stopLocator);
            RegionalConnectorAnchorCount = anchors.Count;

            List<RegionalConnectorCluster> clusters = BuildRegionalConnectorClusters(anchors, cfg);
            RegionalConnectorClusterCount = clusters.Count;
            if (clusters.Count < Mathf.Max(3, cfg.MinStopsPerRoute))
                return routes;

            var usedClusterIndexes = new HashSet<int>();
            int routeLimit = GetRegionalConnectorRouteLimit(clusters.Count, publishedLines);
            for (int routeIndex = 0; routeIndex < routeLimit; routeIndex++)
            {
                List<int> routeClusterIndexes;
                bool builtIndexes = routeIndex == 0
                    ? TryBuildFirstRegionalConnectorClusterIndexes(clusters, cfg, stopLocator, out routeClusterIndexes)
                    : TryBuildAdditionalRegionalConnectorClusterIndexes(clusters, usedClusterIndexes, cfg, stopLocator, out routeClusterIndexes);

                if (!builtIndexes)
                    break;

                TryAddRegionalConnectorClusterStops(routeClusterIndexes, clusters, usedClusterIndexes, cfg, routeIndex > 0);

                List<Vector3> route = BuildRegionalConnectorRoute(routeClusterIndexes, clusters, cfg);
                string rejectionReason;
                if (!TryValidateRegionalConnectorRoute(route, clusters, cfg, stopLocator, out rejectionReason))
                {
                    RegionalConnectorRejectedRouteCount++;
                    if (routeIndex == 0)
                        break;
                ...
                routes.Add(route);
                MarkRegionalConnectorClustersUsed(route, clusters, usedClusterIndexes, cfg);
            }

            RegionalConnectorRouteCount = routes.Count;
            return routes;
        }

        private void ResetRegionalConnectorDiagnostics()
        {
            RegionalConnectorRouteCount = 0;
            RegionalConnectorAnchorCount = 0;
            RegionalConnectorClusterCount = 0;
            RegionalConnectorTransitHubAnchorCount = 0;
            RegionalConnectorRejectedRouteCount = 0;
        }

        private List<RegionalConnectorAnchor> BuildRegionalConnectorAnchors(
            List<ExistingLineSnapshot> publishedLines,
            List<Vector3> transitHubs,
            AutoPublicTransitConfig cfg,
            BusStopLocator stopLocator)
        {
            var anchors = new List<RegionalConnectorAnchor>();
            if (publishedLines != null)
            {
                for (int lineIndex = 0; lineIndex < publishedLines.Count && anchors.Count < MaxRegionalConnectorAnchors; lineIndex++)
                {
                    AddRegionalConnectorLineAnchors(anchors, publishedLines[lineIndex], cfg, stopLocator);
                }
            }

            if (transitHubs != null)
            {
                float hubSearchDistance = Mathf.Max(cfg.MaxRoadDistance * 1.8f, Mathf.Max(cfg.MaxWalkingDistance * 4f, 900f));
                for (int i = 0; i < transitHubs.Count && anchors.Count < MaxRegionalConnectorAnchors; i++)
                {
                    CachedStopMatch match;
                    if (!stopLocator.TryGetNearestBusStopMatch(transitHubs[i], hubSearchDistance, out match))
                        continue;

                    AddRegionalConnectorAnchor(anchors, match.StopPosition, match.SegmentId, 240f, 0, true, cfg);
                    RegionalConnectorTransitHubAnchorCount++;
                }
            }
        ...
        private void AddRegionalConnectorLineAnchors(
            List<RegionalConnectorAnchor> anchors,
            ExistingLineSnapshot line,
            AutoPublicTransitConfig cfg,
            BusStopLocator stopLocator)
        {
            if (anchors == null || line == null || line.Stops == null || line.Stops.Count < 2)
                return;

            List<Vector3> stops = NormalizeOpenRoute(line.Stops);
            if (stops.Count < 2)
                return;

            int firstIndex;
            int secondIndex;
            if (!TryFindFarthestStopPair(stops, out firstIndex, out secondIndex))
                return;

            float baseWeight = 95f + Mathf.Min(120f, stops.Count * 5f) + Mathf.Min(120f, line.StrategicStopCount * 30f);
            if (line.IsProtectedFromAptManagement)
                baseWeight *= 0.65f;

            AddRegionalConnectorLineStopAnchor(anchors, stops[firstIndex], line.LineId, baseWeight, cfg, stopLocator);
            AddRegionalConnectorLineStopAnchor(anchors, stops[secondIndex], line.LineId, baseWeight, cfg, stopLocator);

            if (stops.Count >= 5 && anchors.Count < MaxRegionalConnectorAnchors)
            {
                int representativeIndex = FindRepresentativeLineStopIndex(stops, stops[firstIndex], stops[secondIndex], cfg);
                if (representativeIndex >= 0)
                    AddRegionalConnectorLineStopAnchor(anchors, stops[representativeIndex], line.LineId, baseWeight * 0.72f, cfg, stopLocator);
            }
        }

        private void AddRegionalConnectorLineStopAnchor(
            List<RegionalConnectorAnchor> anchors,
            Vector3 stop,
            ushort lineId,
            float weight,
            ...
            BusStopLocator stopLocator)
        {
            if (anchors.Count >= MaxRegionalConnectorAnchors)
                return;

            CachedStopMatch match;
            float snapDistance = Mathf.Max(80f, cfg.MaxWalkingDistance * 0.45f);
            if (!stopLocator.TryGetNearestBusStopMatch(stop, snapDistance, out match))
                return;

            AddRegionalConnectorAnchor(anchors, match.StopPosition, match.SegmentId, weight, lineId, false, cfg);
        }

        private void AddRegionalConnectorAnchor(
            List<RegionalConnectorAnchor> anchors,
            Vector3 position,
            ushort segmentId,
            float weight,
            ...
            AutoPublicTransitConfig cfg)
        {
            float mergeDistance = Mathf.Max(45f, cfg.GridCellSize * 0.35f);
            for (int i = 0; i < anchors.Count; i++)
            {
                if (Geometry.DistanceXZ(anchors[i].Position, position) > mergeDistance)
                    continue;

                anchors[i].Weight += weight;
                if (transitHub)
                    anchors[i].IsTransitHub = true;
                if (lineId != 0)
                    anchors[i].LineIds.Add(lineId);
                if (weight > anchors[i].RepresentativeWeight)
                {
                    anchors[i].Position = position;
                    anchors[i].SegmentId = segmentId;
                    anchors[i].RepresentativeWeight = weight;
                }

                return;
            }

            var anchor = new RegionalConnectorAnchor
            {
                Position = position,
                SegmentId = segmentId,
                Weight = weight,
                RepresentativeWeight = weight,
                IsTransitHub = transitHub
            };
            if (lineId != 0)
                anchor.LineIds.Add(lineId);

            anchors.Add(anchor);
        }

        private bool TryFindFarthestStopPair(List<Vector3> stops, out int firstIndex, out int secondIndex)
        {
            firstIndex = -1;
            secondIndex = -1;
            ...
            for (int i = 0; i < stops.Count; i++)
            {
                ...
                {
                    float distance = Geometry.DistanceXZ(stops[i], stops[j]);
        ...
        private int FindRepresentativeLineStopIndex(List<Vector3> stops, Vector3 first, Vector3 second, AutoPublicTransitConfig cfg)
        {
            Vector3 center = Vector3.zero;
            for (int i = 0; i < stops.Count; i++)
                center += stops[i];

            center /= Mathf.Max(1, stops.Count);

            int bestIndex = -1;
            float bestScore = float.MaxValue;
            float minimumEndpointDistance = Mathf.Max(cfg.MaxWalkingDistance * 1.25f, cfg.GridCellSize);
            for (int i = 0; i < stops.Count; i++)
            {
                if (Geometry.DistanceXZ(stops[i], first) < minimumEndpointDistance ||
                    Geometry.DistanceXZ(stops[i], second) < minimumEndpointDistance)
                {
                    continue;
                }

                float score = Geometry.DistanceXZ(stops[i], center);
                if (score >= bestScore)
                    continue;
        ...
        private List<RegionalConnectorCluster> BuildRegionalConnectorClusters(List<RegionalConnectorAnchor> anchors, AutoPublicTransitConfig cfg)
        {
            var clusters = new List<RegionalConnectorCluster>();
            if (anchors == null)
                return clusters;

            float clusterDistance = GetRegionalConnectorClusterDistance(cfg);
            for (int i = 0; i < anchors.Count; i++)
            {
                AddAnchorToRegionalConnectorClusters(clusters, anchors[i], clusterDistance);
            }
        ...
        private void AddAnchorToRegionalConnectorClusters(
            List<RegionalConnectorCluster> clusters,
            RegionalConnectorAnchor anchor,
            float clusterDistance)
        {
            int bestIndex = -1;
            float bestDistance = clusterDistance;
            for (int i = 0; i < clusters.Count; i++)
            {
                float distance = Geometry.DistanceXZ(clusters[i].Position, anchor.Position);
                if (distance >= bestDistance)
                    continue;
            ...
            if (bestIndex >= 0)
            {
                clusters[bestIndex].Add(anchor);
                return;
            }

            var cluster = new RegionalConnectorCluster();
            cluster.Index = clusters.Count;
            cluster.Add(anchor);
            clusters.Add(cluster);
        }

        private int GetRegionalConnectorRouteLimit(int clusterCount, List<ExistingLineSnapshot> publishedLines)
        {
            int usefulLineCount = 0;
            if (publishedLines != null)
            {
                for (int i = 0; i < publishedLines.Count; i++)
                {
                    if (publishedLines[i] != null && publishedLines[i].Stops != null && publishedLines[i].Stops.Count >= 2)
                        usefulLineCount++;
                }
            }

            if (clusterCount < 7 || usefulLineCount < 6)
                return 1;

            if (clusterCount < 12 || usefulLineCount < 12)
                return 2;

            return Mathf.Min(MaxRegionalConnectorRoutes, 3);
        }

        private bool TryBuildFirstRegionalConnectorClusterIndexes(
            List<RegionalConnectorCluster> clusters,
            AutoPublicTransitConfig cfg,
            BusStopLocator stopLocator,
            out List<int> routeClusterIndexes)
        {
            routeClusterIndexes = new List<int>();
            int first;
            int second;
            if (!TryFindFarthestRegionalConnectorPair(clusters, null, cfg, out first, out second))
                return false;

            routeClusterIndexes.Add(first);
            routeClusterIndexes.Add(second);
            TryAddRegionalConnectorClusterStops(routeClusterIndexes, clusters, null, cfg, false);
            return routeClusterIndexes.Count >= Mathf.Max(3, cfg.MinStopsPerRoute);
        }

        private bool TryBuildAdditionalRegionalConnectorClusterIndexes(
            List<RegionalConnectorCluster> clusters,
            HashSet<int> usedClusterIndexes,
            AutoPublicTransitConfig cfg,
            BusStopLocator stopLocator,
            out List<int> routeClusterIndexes)
        {
            routeClusterIndexes = new List<int>();
            if (usedClusterIndexes == null || usedClusterIndexes.Count == 0)
                return TryBuildFirstRegionalConnectorClusterIndexes(clusters, cfg, stopLocator, out routeClusterIndexes);

            int remoteIndex = FindFarthestUnusedRegionalConnectorCluster(clusters, usedClusterIndexes, cfg);
            if (remoteIndex < 0)
                return false;

            int partnerIndex = FindBestDistantUnusedRegionalConnectorCluster(remoteIndex, clusters, usedClusterIndexes, cfg);
            if (partnerIndex < 0)
                return false;

            int transferIndex = FindNearestUsedRegionalConnectorCluster(remoteIndex, clusters, usedClusterIndexes);
            if (transferIndex < 0)
                return false;

            routeClusterIndexes.Add(transferIndex);
            routeClusterIndexes.Add(remoteIndex);
            routeClusterIndexes.Add(partnerIndex);
            TryAddRegionalConnectorClusterStops(routeClusterIndexes, clusters, usedClusterIndexes, cfg, true);
            return routeClusterIndexes.Count >= Mathf.Max(3, cfg.MinStopsPerRoute);
        }

        private bool TryFindFarthestRegionalConnectorPair(
            List<RegionalConnectorCluster> clusters,
            ...
            out int second)
        {
            ...
            second = -1;
            float bestScore = 0f;
            float minimumSpan = GetRegionalConnectorMinimumSpan(cfg);
            for (int i = 0; i < clusters.Count; i++)
            {
                ...
                {
                    if (excludedIndexes != null && excludedIndexes.Contains(j))
                        continue;

                    float distance = Geometry.DistanceXZ(clusters[i].Position, clusters[j].Position);
                    if (distance < minimumSpan)
                        continue;

                    float score = distance + Mathf.Min(420f, clusters[i].Weight + clusters[j].Weight) * 0.35f;
                    if (clusters[i].IsTransitHub || clusters[j].IsTransitHub)
                        score += 260f;
        ...
        private void TryAddRegionalConnectorClusterStops(
            List<int> routeClusterIndexes,
            List<RegionalConnectorCluster> clusters,
            HashSet<int> usedClusterIndexes,
            AutoPublicTransitConfig cfg,
            bool preferUnused)
        {
            if (routeClusterIndexes == null || clusters == null)
                return;

            int targetStops = GetRegionalConnectorTargetStops(clusters.Count, cfg);
            int safety = 0;
            while (routeClusterIndexes.Count < targetStops && safety < clusters.Count * 2)
            {
                safety++;
                int nextIndex = FindBestRegionalConnectorAddition(routeClusterIndexes, clusters, usedClusterIndexes, cfg, preferUnused);
                if (nextIndex < 0)
                    break;

                routeClusterIndexes.Add(nextIndex);
            }
        }

        private int FindBestRegionalConnectorAddition(
            List<int> routeClusterIndexes,
            List<RegionalConnectorCluster> clusters,
            HashSet<int> usedClusterIndexes,
            AutoPublicTransitConfig cfg,
            bool preferUnused)
        {
            ...
            float minimumSpacing = GetRegionalConnectorClusterDistance(cfg) * 1.15f;
            for (int i = 0; i < clusters.Count; i++)
            {
                if (routeClusterIndexes.Contains(i))
                    continue;

                bool alreadyUsed = usedClusterIndexes != null && usedClusterIndexes.Contains(i);
                if (preferUnused && alreadyUsed)
                    continue;

                float nearestRouteDistance = GetNearestClusterDistanceToRoute(i, routeClusterIndexes, clusters);
                if (nearestRouteDistance < minimumSpacing)
                    continue;

                float score = nearestRouteDistance + Mathf.Min(360f, clusters[i].Weight) * 0.45f;
                if (clusters[i].IsTransitHub)
                    score += 240f;
                if (alreadyUsed)
                    score -= 520f;

                float projectedLength = EstimateRegionalConnectorDirectLength(routeClusterIndexes, i, clusters, cfg);
                if (projectedLength > cfg.MaxLineLengthKm * 1000f * 1.05f)
                    score -= 900f;
        ...
        private int FindFarthestUnusedRegionalConnectorCluster(
            List<RegionalConnectorCluster> clusters,
            HashSet<int> usedClusterIndexes,
            AutoPublicTransitConfig cfg)
        {
            int bestIndex = -1;
            float bestScore = 0f;
            float minimumDistance = GetRegionalConnectorMinimumSpan(cfg) * 0.85f;
            for (int i = 0; i < clusters.Count; i++)
            {
                if (usedClusterIndexes.Contains(i))
                    continue;

                float nearestUsedDistance = GetNearestUsedClusterDistance(i, clusters, usedClusterIndexes);
                if (nearestUsedDistance < minimumDistance)
                    continue;

                float score = nearestUsedDistance + Mathf.Min(320f, clusters[i].Weight) * 0.35f;
                if (clusters[i].IsTransitHub)
                    score += 180f;
        ...
        private int FindBestDistantUnusedRegionalConnectorCluster(
            int fromIndex,
            List<RegionalConnectorCluster> clusters,
            HashSet<int> usedClusterIndexes,
            AutoPublicTransitConfig cfg)
        {
            int bestIndex = -1;
            float bestScore = 0f;
            float minimumDistance = GetRegionalConnectorMinimumSpan(cfg) * 0.6f;
            for (int i = 0; i < clusters.Count; i++)
            {
                if (i == fromIndex || usedClusterIndexes.Contains(i))
                    continue;

                float distance = Geometry.DistanceXZ(clusters[fromIndex].Position, clusters[i].Position);
                if (distance < minimumDistance)
                    continue;

                float score = distance + Mathf.Min(320f, clusters[i].Weight) * 0.3f;
                if (clusters[i].IsTransitHub)
                    score += 160f;
        ...
        private int FindNearestUsedRegionalConnectorCluster(
            int fromIndex,
            List<RegionalConnectorCluster> clusters,
            HashSet<int> usedClusterIndexes)
        {
            ...
            foreach (int usedIndex in usedClusterIndexes)
            {
                if (usedIndex < 0 || usedIndex >= clusters.Count)
                    continue;

                float distance = Geometry.DistanceXZ(clusters[fromIndex].Position, clusters[usedIndex].Position);
                if (distance >= bestDistance)
                    continue;
        ...
        private float GetNearestUsedClusterDistance(int clusterIndex, List<RegionalConnectorCluster> clusters, HashSet<int> usedClusterIndexes)
        {
            float bestDistance = float.MaxValue;
            foreach (int usedIndex in usedClusterIndexes)
            {
                ...
                bestDistance = Mathf.Min(bestDistance, Geometry.DistanceXZ(clusters[clusterIndex].Position, clusters[usedIndex].Position));
            }

            return bestDistance;
        }

        private float GetNearestClusterDistanceToRoute(int clusterIndex, List<int> routeClusterIndexes, List<RegionalConnectorCluster> clusters)
        {
            float bestDistance = float.MaxValue;
            for (int i = 0; i < routeClusterIndexes.Count; i++)
            {
                int routeIndex = routeClusterIndexes[i];
                if (routeIndex < 0 || routeIndex >= clusters.Count)
                    continue;

                bestDistance = Mathf.Min(bestDistance, Geometry.DistanceXZ(clusters[clusterIndex].Position, clusters[routeIndex].Position));
            }

            return bestDistance;
        }

        private List<Vector3> BuildRegionalConnectorRoute(
            List<int> routeClusterIndexes,
            List<RegionalConnectorCluster> clusters,
            AutoPublicTransitConfig cfg)
        {
            ...
            for (int i = 0; i < routeClusterIndexes.Count; i++)
            {
                int clusterIndex = routeClusterIndexes[i];
                if (clusterIndex < 0 || clusterIndex >= clusters.Count)
                    continue;

                AddDistinctRegionalConnectorStop(route, clusters[clusterIndex].Position, cfg);
            }

            route = RouteShapeAnalyzer.OptimizeClosedLoop(route, cfg);
            return NormalizeOpenRoute(route);
        }

        private bool AddDistinctRegionalConnectorStop(List<Vector3> route, Vector3 stop, AutoPublicTransitConfig cfg)
        {
            float minDistance = Mathf.Max(75f, cfg.GridCellSize * 0.55f);
            for (int i = 0; i < route.Count; i++)
            {
                if (Geometry.DistanceXZ(route[i], stop) < minDistance)
                    return false;
            }
        ...
        private bool TryValidateRegionalConnectorRoute(
            List<Vector3> route,
            List<RegionalConnectorCluster> clusters,
            AutoPublicTransitConfig cfg,
            BusStopLocator stopLocator,
            out string rejectionReason)
        {
            ...
            if (route == null || route.Count < Mathf.Max(3, cfg.MinStopsPerRoute))
            {
                rejectionReason = "too-few-stops";
                return false;
            }

            if (ComputeRouteSpan(route) < GetRegionalConnectorMinimumSpan(cfg))
            {
                rejectionReason = "too-local";
                return false;
            }

            int servedLineClusters = CountRegionalConnectorServedLineClusters(route, clusters, cfg);
            if (servedLineClusters < 3)
            {
                rejectionReason = "too-few-line-pockets";
                return false;
            }

            string shapeReason;
            if (RouteShapeAnalyzer.TryGetShapeProblem(route, cfg, out shapeReason))
            {
                rejectionReason = shapeReason;
                return false;
            }

            float routeLength = ComputeRegionalConnectorLength(route, cfg, stopLocator);
            if (routeLength == float.MaxValue || routeLength > cfg.MaxLineLengthKm * 1000f * 1.15f)
            {
        ...
        private int CountRegionalConnectorServedLineClusters(List<Vector3> route, List<RegionalConnectorCluster> clusters, AutoPublicTransitConfig cfg)
        {
            var served = new HashSet<int>();
            float transferDistance = GetRegionalConnectorClusterDistance(cfg) * 1.15f;
            for (int i = 0; i < route.Count; i++)
            {
                int clusterIndex = FindNearestRegionalConnectorCluster(route[i], clusters, transferDistance);
                if (clusterIndex < 0)
                    continue;

                if (clusters[clusterIndex].LineIds.Count > 0)
                    served.Add(clusterIndex);
            }

            return served.Count;
        }

        private int FindNearestRegionalConnectorCluster(Vector3 stop, List<RegionalConnectorCluster> clusters, float maxDistance)
        {
            int bestIndex = -1;
            float bestDistance = maxDistance;
            for (int i = 0; i < clusters.Count; i++)
            {
                float distance = Geometry.DistanceXZ(stop, clusters[i].Position);
                if (distance >= bestDistance)
                    continue;
        ...
        private void MarkRegionalConnectorClustersUsed(
            List<Vector3> route,
            List<RegionalConnectorCluster> clusters,
            HashSet<int> usedClusterIndexes,
            AutoPublicTransitConfig cfg)
        ...
        private float ComputeRegionalConnectorLength(List<Vector3> route, AutoPublicTransitConfig cfg, BusStopLocator stopLocator)
        {
            ...
            {
                float legDistance;
                if (!TryGetRegionalConnectorLegDistance(route[i], route[(i + 1) % route.Count], cfg, stopLocator, out legDistance))
        ...
        private bool TryGetRegionalConnectorLegDistance(
            Vector3 from,
            Vector3 to,
            AutoPublicTransitConfig cfg,
            BusStopLocator stopLocator,
            out float distance)
        {
            ...
            float fallbackLimit = Mathf.Max(2600f, cfg.MaxRoadDistance * 4.8f);
            float roadSearchLimit = Mathf.Max(5200f, cfg.MaxLineLengthKm * 1000f * 0.75f);
            distance = direct;

            CachedStopMatch fromMatch;
            CachedStopMatch toMatch;
            if (stopLocator != null &&
                stopLocator.TryGetNearestBusStopMatch(from, Mathf.Max(96f, cfg.MaxWalkingDistance * 0.55f), out fromMatch) &&
                stopLocator.TryGetNearestBusStopMatch(to, Mathf.Max(96f, cfg.MaxWalkingDistance * 0.55f), out toMatch))
            {
                float roadDistance;
                if (stopLocator.TryEstimateRoadDistance(fromMatch.SegmentId, toMatch.SegmentId, roadSearchLimit, out roadDistance))
                {
                    distance = Mathf.Max(direct, roadDistance);
                    return distance <= roadSearchLimit;
                }
            }

            if (direct <= fallbackLimit)
            {
                distance = direct * 1.25f;
                return true;
            }
        ...
        private float EstimateRegionalConnectorDirectLength(
            List<int> routeClusterIndexes,
            int candidateIndex,
            List<RegionalConnectorCluster> clusters,
            AutoPublicTransitConfig cfg)
        {
            ...
            for (int i = 0; i < routeClusterIndexes.Count; i++)
            {
                int routeIndex = routeClusterIndexes[i];
                if (routeIndex >= 0 && routeIndex < clusters.Count)
                    route.Add(clusters[routeIndex].Position);
            }

            if (candidateIndex >= 0 && candidateIndex < clusters.Count)
                route.Add(clusters[candidateIndex].Position);

            route = RouteShapeAnalyzer.OptimizeClosedLoop(route, cfg);
            return RouteShapeAnalyzer.ComputeClosedLength(route);
        }

        private int GetRegionalConnectorTargetStops(int clusterCount, AutoPublicTransitConfig cfg)
        {
            int minimum = Mathf.Max(4, cfg.MinStopsPerRoute);
            int target = clusterCount >= 14 ? 5 : (clusterCount >= 8 ? 5 : 4);
            return Mathf.Min(Mathf.Max(minimum, target), Mathf.Min(cfg.MaxStopsPerRoute, 6));
        }

        private float GetRegionalConnectorClusterDistance(AutoPublicTransitConfig cfg)
        {
            return Mathf.Max(360f, Mathf.Max(cfg.MaxWalkingDistance * 1.55f, cfg.GridCellSize * 1.85f));
        }

        private float GetRegionalConnectorMinimumSpan(AutoPublicTransitConfig cfg)
        {
            return Mathf.Max(1400f, Mathf.Max(cfg.MaxRoadDistance * 2.4f, cfg.MaxWalkingDistance * 5.5f));
        }
        ...
        {
            if (IsMajorTouristNode(seed, cfg))
                return GetMajorTouristDistanceOverride(cfg);
        ...
        {
            if (IsMajorTouristNode(seed, cfg))
                return GetMajorTouristRouteLengthLimit(cfg);
        ...
        {
            if (IsMajorTouristNode(node, cfg))
                return GetMajorTouristDistanceOverride(cfg);
            ...
            out List<Vector3> route)
        {
            ...
            float maxBackfillLength = GetCoverageBackfillMaxRouteLength(nodes[seedIndex], cfg);
            ...
            BusStopLocator stopLocator)
        {
            ...
            float maxBackfillLength = GetCoverageBackfillMaxRouteLength(seed, cfg);
        ...
        private float GetCoverageBackfillMaxRouteLength(DemandNode seed, AutoPublicTransitConfig cfg)
        {
            if (IsMajorTouristNode(seed, cfg))
                return GetMajorTouristRouteLengthLimit(cfg);
            ...
            BusStopLocator stopLocator)
        {
            ...
            Vector3 current = route[route.Count - 1];
            int currentIndex = FindNodeIndexForStop(current, nodes);
            float localReach = GetTouristCoverageLocalReach(seed, cfg);
        ...
        private bool IsMajorTouristNode(DemandNode node, AutoPublicTransitConfig cfg)
        {
            return IsTouristNode(node) && node.Demand >= GetMajorTouristDemandThreshold(cfg);
        }

        private int GetMajorTouristDemandThreshold(AutoPublicTransitConfig cfg)
        {
            return cfg != null ? Mathf.Max(cfg.DemandThreshold * 12, 60) : 60;
        }

        private float GetMajorTouristDistanceOverride(AutoPublicTransitConfig cfg)
        {
            if (cfg == null)
                return 4200f;

            return Mathf.Min(
                cfg.MaxLineLengthKm * 1000f * 0.45f,
                Mathf.Max(4200f, Mathf.Max(cfg.MaxRoadDistance * 7.0f, cfg.MaxWalkingDistance * 16.0f)));
        }

        private float GetMajorTouristRouteLengthLimit(AutoPublicTransitConfig cfg)
        {
            if (cfg == null)
                return 7200f;
            ...
            return Mathf.Min(
                ...
                Mathf.Max(7200f, Mathf.Max(cfg.MaxRoadDistance * 12.0f, cfg.MaxWalkingDistance * 24.0f)));
        }
        ...
        private float GetTouristCoverageLocalReach(DemandNode seed, AutoPublicTransitConfig cfg)
        {
            if (IsMajorTouristNode(seed, cfg))
                return GetMajorTouristDistanceOverride(cfg);
        ...
        {
            if (IsMajorTouristNode(fromNode, cfg) || IsMajorTouristNode(toNode, cfg))
                return GetMajorTouristDistanceOverride(cfg);
        ...
        private class RegionalConnectorAnchor
        {
            public Vector3 Position;
            public ushort SegmentId;
            public float Weight;
            public float RepresentativeWeight;
            public bool IsTransitHub;
            public readonly HashSet<ushort> LineIds = new HashSet<ushort>();
        }

        private class RegionalConnectorCluster
        {
            public int Index;
            public Vector3 Position;
            public ushort SegmentId;
            ...
            public void Add(RegionalConnectorAnchor anchor)
            {
                if (anchor == null)
                    return;
                ...
                AnchorCount++;
                if (anchor.IsTransitHub)
                    IsTransitHub = true;

                foreach (ushort lineId in anchor.LineIds)
                    LineIds.Add(lineId);

                if (AnchorCount == 1 || anchor.RepresentativeWeight > RepresentativeWeight)
                {
                    Position = anchor.Position;
                    SegmentId = anchor.SegmentId;
                    RepresentativeWeight = anchor.RepresentativeWeight;
                }
            }
        }
    }
