...
namespace AutoPublicTransit
{
    public partial class Manager : MonoBehaviour
    {
            ...
            BusStopLocator stopLocator)
        {
            ...
            int isolatedFallbackInjected = 0;
            ...
            float isolatedFallbackRoadDistance = GetIsolatedTouristAnchorFallbackRoadDistance(cfg);
            ...
            for (int i = 0; i < anchors.Count; i++)
            {
                ...
                {
                    float rescueDistance = IsMajorTouristAnchor(anchor, cfg)
                        ? isolatedFallbackRoadDistance
                    ...
                    if (rescueDistance > fallbackRoadDistance + 1f)
                        isolatedFallbackInjected++;
                    else
                        fallbackInjected++;
                }
            ...
            TransitLogging.Log(
                ...
                ", coveredPotentialKept=" + coveredCapacityKept +
                ", noRoad=" + noRoad +
                ", accessPositionSnaps=" + accessPositionSnaps +
                ", isolatedFallbackInjected=" + isolatedFallbackInjected +
                ...
                ", isolatedFallbackRoadDistance=" + isolatedFallbackRoadDistance.ToString("0", CultureInfo.InvariantCulture) + ".");
        }
        ...
        private void InjectZonedRciDemand(
            DemandGrid demandGrid,
            List<Vector3> existingStops,
            AutoPublicTransitConfig cfg,
            BusStopLocator stopLocator)
        {
            if (demandGrid == null || cfg == null || stopLocator == null)
                return;

            int zoneReadSkipped;
            Dictionary<long, ZonedRciAggregate> aggregates = CollectZonedRciAggregates(cfg, out zoneReadSkipped);
            if (aggregates.Count == 0)
            {
                TransitLogging.Log("Zoned RCI demand injection: zonedCells=0, aggregates=0, injected=0, zoneReadSkipped=" + zoneReadSkipped + ".");
                return;
            }
            ...
            int noRoad = 0;
            int injectedDemandTotal = 0;
            int residentialCells = 0;
            int commercialCells = 0;
            int officeCells = 0;
            int industrialCells = 0;
            float searchDistance = GetZonedRciBusStopSearchDistance(cfg);

            foreach (var pair in aggregates)
            {
                ZonedRciAggregate aggregate = pair.Value;
                Vector3 position = aggregate.SumPosition / Mathf.Max(1, aggregate.CellCount);
                int demandWeight = GetZonedRciDemandWeight(aggregate, cfg);
                int discountedDemand = ApplyExistingCoverageDiscount(demandWeight, position, existingStops, cfg);
                if (discountedDemand <= 0)
                {
                    alreadyCovered++;
                    continue;
                ...
                CachedStopMatch stopMatch;
                if (!stopLocator.TryGetNearestBusStopMatch(position, searchDistance, out stopMatch))
                {
                    noRoad++;
                    continue;
                }

                demandGrid.AddSample(position, stopMatch, discountedDemand, aggregate.Purpose);
                injectedDemandTotal += discountedDemand;
                injected++;

                switch (aggregate.Purpose)
                {
                    case DemandNodePurpose.Residential:
                        residentialCells += aggregate.CellCount;
                        break;
                    case DemandNodePurpose.Commercial:
                        commercialCells += aggregate.CellCount;
                        break;
                    case DemandNodePurpose.Office:
                        officeCells += aggregate.CellCount;
                        break;
                    case DemandNodePurpose.Industrial:
                        industrialCells += aggregate.CellCount;
                        break;
                }
            }

            TransitLogging.Log(
                "Zoned RCI demand injection: zonedCells=" + CountZonedRciCells(aggregates) +
                ", aggregates=" + aggregates.Count +
                ", injected=" + injected +
                ", alreadyCovered=" + alreadyCovered +
                ", noRoad=" + noRoad +
                ", residentialCells=" + residentialCells +
                ", commercialCells=" + commercialCells +
                ", officeCells=" + officeCells +
                ", industrialCells=" + industrialCells +
                ", injectedDemandTotal=" + injectedDemandTotal +
                ", zoneReadSkipped=" + zoneReadSkipped +
                ", searchDistance=" + searchDistance.ToString("0", CultureInfo.InvariantCulture) + ".");
        }

        private Dictionary<long, ZonedRciAggregate> CollectZonedRciAggregates(AutoPublicTransitConfig cfg, out int zoneReadSkipped)
        {
            zoneReadSkipped = 0;
            var aggregates = new Dictionary<long, ZonedRciAggregate>();
            ZoneManager zoneManager = ZoneManager.instance;
            if (zoneManager == null)
                return aggregates;

            ZoneBlock[] blocks = zoneManager.m_blocks.m_buffer;
            if (blocks == null)
                return aggregates;
            ...
            for (ushort blockId = 1; blockId < blocks.Length; blockId++)
            {
                ...
                try
                {
                    CollectZonedRciBlockAggregates(block, cfg, aggregates);
                }
                catch (InvalidOperationException e)
                {
                    zoneReadSkipped++;
                    TransitLogging.Warn("Skipped zoned RCI block " + blockId + " because the game reported zone data out of sync: " + e.Message);
                }
            }
        ...
        private void CollectZonedRciBlockAggregates(
            ZoneBlock block,
            AutoPublicTransitConfig cfg,
            Dictionary<long, ZonedRciAggregate> aggregates)
        {
            if (aggregates == null)
                return;
            ...
            for (int x = 0; x < 8; x++)
            {
                ...
                {
                    int cellIndex = z * 8 + x;
                    if (((block.m_valid >> cellIndex) & 1UL) == 0UL)
                        continue;

                    ItemClass.Zone zone = block.GetZone(x, z);
                    int purpose = GetZonedRciPurpose(zone);
                    if (purpose == DemandNodePurpose.Normal)
                        continue;

                    Vector3 position = GetZoneCellPosition(block, x, z);
                    int cellX = Mathf.FloorToInt(position.x / cfg.GridCellSize);
                    int cellZ = Mathf.FloorToInt(position.z / cfg.GridCellSize);
                    long key = GetZonedRciAggregateKey(cellX, cellZ, purpose);

                    ZonedRciAggregate aggregate;
                    if (!aggregates.TryGetValue(key, out aggregate))
                    {
                        aggregate = new ZonedRciAggregate
                        {
                            Purpose = purpose,
                            ZoneWeight = GetZonedRciCellWeight(zone)
                        };
                    }

                    aggregate.SumPosition += position;
                    aggregate.CellCount++;
                    aggregate.ZoneWeight = Mathf.Max(aggregate.ZoneWeight, GetZonedRciCellWeight(zone));
                    aggregates[key] = aggregate;
                }
            }
        }

        private Vector3 GetZoneCellPosition(ZoneBlock block, int x, int z)
        {
            const float zoneCellSize = 8f;
            float angle = block.m_angle;
            Vector3 forward = new Vector3(Mathf.Cos(angle), 0f, Mathf.Sin(angle));
            Vector3 right = new Vector3(forward.z, 0f, -forward.x);
            return block.m_position
                + forward * ((x - 3.5f) * zoneCellSize)
                + right * ((z + 0.5f) * zoneCellSize);
        }

        private int GetZonedRciPurpose(ItemClass.Zone zone)
        {
            switch (zone)
            {
        ...
        private int GetZonedRciCellWeight(ItemClass.Zone zone)
        {
            switch (zone)
            {
                case ItemClass.Zone.ResidentialHigh:
        ...
        private int GetZonedRciDemandWeight(ZonedRciAggregate aggregate, AutoPublicTransitConfig cfg)
        {
            int minimum = Mathf.Max(cfg.DemandThreshold + 3, 8);
            int weighted = aggregate.CellCount * Mathf.Max(1, aggregate.ZoneWeight);
            return Mathf.Max(minimum, Mathf.Min(160, weighted));
        }

        private float GetZonedRciBusStopSearchDistance(AutoPublicTransitConfig cfg)
        {
            return Mathf.Max(cfg.MaxWalkingDistance * 1.8f, cfg.MaxRoadDistance * 0.85f);
        }

        private long GetZonedRciAggregateKey(int cellX, int cellZ, int purpose)
        {
            long purposePart = ((long)purpose & 0xffL) << 56;
            long xPart = ((long)cellX & 0x0fffffffL) << 28;
            long zPart = (long)cellZ & 0x0fffffffL;
            return purposePart ^ xPart ^ zPart;
        }

        private int CountZonedRciCells(Dictionary<long, ZonedRciAggregate> aggregates)
        {
            int total = 0;
            foreach (var pair in aggregates)
                total += pair.Value.CellCount;
        ...
        private float GetIsolatedTouristAnchorFallbackRoadDistance(AutoPublicTransitConfig cfg)
        {
            if (cfg == null)
                return 4200f;
            ...
            return Mathf.Min(
                ...
                Mathf.Max(4200f, Mathf.Max(cfg.MaxRoadDistance * 7f, cfg.MaxWalkingDistance * 16f)));
        }

        private bool IsMajorTouristAnchor(TouristAnchorInfo anchor, AutoPublicTransitConfig cfg)
        {
            int threshold = cfg != null ? Mathf.Max(cfg.DemandThreshold * 12, 60) : 60;
            return anchor.DemandWeight >= threshold ||
                anchor.VisitorCapacity >= threshold ||
                anchor.Category == TouristAnchorCategory.Monument ||
                anchor.Category == TouristAnchorCategory.UniqueBuilding;
        }
        ...
        private struct ZonedRciAggregate
        {
            public Vector3 SumPosition;
            public int CellCount;
            public int Purpose;
            public int ZoneWeight;
        }
